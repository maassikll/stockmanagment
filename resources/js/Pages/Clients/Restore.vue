<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            ...
            Restauration des Clients supprimés
        </template>



        <div v-if="restoreClients && restoreClients.data && restoreClients.data.length" class="inline-block min-w-full overflow-hidden rounded-lg shadow">
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody>
                    <tr v-for="client in restoreClients.data" :key="client.id" class="text-gray-700">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">{{ truncateText(client.deleted_at, 20) }}</p>
                        </td>

                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <form @submit.prevent=" restoreClient(client.id)">
            ...
            <div class="flex flex-col items-center border-t bg-white px-5 py-5 xs:flex-row xs:justify-between">
                <pagination :links="restoreClients.links" />
            </div>
        </div>
        <div v-else class="text-center py-10">
            <p class="text-gray-500">Pas de client a réstaurer </p>
        </div>
    </AuthenticatedLayout>
</template>
...
const restoreClient = (id) => {
    form.post(route('clients.clientRestore', id));
};
